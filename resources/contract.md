...
| `journal/<ts>-<sid8>.md` | записи: решения, результаты, тупики, состояние дерева |
...
```text
.agents/state/tasks/<slug>/journal/20260910T142233Z-019a3f7c.md
                                   ├─ UTC, basic ISO, точность до секунды
                                                    └─ первые 8 символов session id
```

Счётчика (`0001-`, `0002-`) нет намеренно: он гоночный — два чата одновременно
вычислят один и тот же следующий номер. Порядок несёт timestamp, уникальность —
session id, поэтому запись не читает соседние файлы и не требует блокировки.
Лексикографическая сортировка имён даёт хронологический порядок. Коллизия возможна
только внутри одной секунды у одной сессии и разрешается локальным суффиксом
`-2`, `-3`; между сессиями гонки нет по построению.

Внутри записи — фронтматтер `session`, `at`, `stage`, дальше текст.

Legacy `journal.md` читается как самая ранняя запись и **не конвертируется**: новые
записи всегда идут в `journal/`.
...
Значение обязано соответствовать `^[A-Za-z0-9._-]{1,128}$`. Не прошедшее — **отказ,
а не санитизация**: приведённый к «безопасному» виду чужой идентификатор молча
склеивает два разных чата в одну привязку. Если id не дал ни один источник, сессия
работает **без привязки** — это законный режим, а не ошибка.

В `.gitignore` исключены ссылка `.agents/memory`, каталог `.agents/state/sessions/`
и `.agents/runs.jsonl`, а не вся `.agents/`. Определения, скиллы и состояние задач
версионируются в основном проекте.
...
перепривязка **только по явной команде**, молча не перезаписывать. Запись атомарная:
временный файл в том же каталоге плюс `os.replace`.

**Чтение журнала целиком** — канонический порядок: сначала legacy `journal.md`, если
он есть, затем `journal/*.md` по возрастанию имени.
...
Механически checkpoint — **создание нового файла** `journal/<ts>-<sid8>.md`
(`agent-system task checkpoint`). Существующие записи не переписываются, соседние
файлы не читаются, блокировка не нужна: имя уникально по session id.
...
Timestamp в имени задаёт порядок, session id — уникальность. Общего счётчика
(`0001-`, `0002-`) нет намеренно: он гоночный.
...
- **Read-modify-write есть только у `task.md`** — смена `status` и правка scope.

...
.agents/state/tasks/<slug>/    состояние задачи    может быть очищено после выполнения задачи
```
...
и знание. Поэтому она лежит вне рабочего дерева — в одном приватном git-репозитории 
на все проекты, а в дерево попадает симлинком:

```text
~/.agents-memory/                     приватное репо, одно на все проекты
├── project-a-memory/                 git-repo of "project a" menory
├── project-b-memory/                 git-repo
└── project-c-memory/                 git-repo

repo/.agents/memory            -> ~/.agents-memory/project-a
../repo-2/.agents/memory -> ~/.agents-memory/project-a   # тот же проект, другой worktree
repo/.worktrees/repo/.agents/memory -> ~/.agents-memory/project-a   # тот же проект, другой worktree
```
...
Checkpointing should be called as a skills in skill scope called checkpoint.
...
- `.agents/state/LOCK` — advisory-пометка: увидел чужой,

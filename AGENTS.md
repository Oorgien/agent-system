...
## Development and distributable skills

Package sources live at the repository root. The CLI installs them into `.agents/`
in the target project:

| Source | Installation path |
|---|---|
| `agents/` — canonical explorer, implementer, reviewer roles | `.agents/agents/` (+ generated `.claude/agents/`, `.codex/agents/`) |
| `templates/` — `task.md`, `entry.md` templates | `.agents/state/templates/` |
| `skills/` — distributable `checkpoint`, `migrate-memory` skills | `.agents/skills/` (the project's other skills are preserved) |
| `resources/contract.md` — this contract | `.agents/agent-system/contract.md` |

In this clone, `.agents/` is the workspace for agents developing the CLI itself
(local skills, memory, task state). The entire directory is gitignored and excluded
from the package, as is the local `.claude/skills` symlink. The versioning rules for
installed definitions and state below apply to projects that use the tool.
...
clone does not inherit Git config: set the key once with `./setup.sh <project>`.

An explicit change of key or store reconfigures **only the current tree**; run
`./setup.sh` again in the others. `python3 tools/check_state.py` detects trees that
still use the old settings and reports an error. Old memory is not moved automatically.

The symlink is gitignored, so run `./setup.sh` **in every worktree and on every
...
this tree has no memory: run `./setup.sh` (§1) instead of silently working without it.
...
```bash
./setup.sh                            # memory store + symlinks (§1), in every tree
python3 tools/check_state.py          # check task state and bindings (§2)
python3 tools/gen_agents.py           # regenerate native definitions
python3 tools/gen_agents.py --check   # drift check; changes nothing
python3 -m unittest discover tools/tests -v
```

Tasks and bindings are managed through CLI subcommands:
...
`task status` and `task list` do not write anything. `task bind` is the only command
that creates a binding; no operation creates one silently.
...
the next generator run overwrites them, and CI fails the drift check. Change
`agents/*.md` and regenerate instead (in an installed project, `.agents/agents/*.md`
and `gen_agents.py --project <root>`).
...
and the requirement to rerun `./setup.sh` in each tree after changing them, are in §1

...
`.agents/skills/` — локальные скиллы для работы агента над этим CLI.
`skills/` — отдельный источник скиллов, устанавливаемых CLI в `.agents/skills/`
целевого проекта. Поставляемые скиллы править в `skills/`; локальные копии
не входят в пакет и автоматически не синхронизируются. Сейчас поставляются только
`checkpoint` и `migrate-memory`; чужие скиллы целевых проектов сохраняются.
В этом клоне вся `.agents/` локальна и gitignored. Описанное ниже версионирование
установленных определений и состояния относится к подключаемым проектам.

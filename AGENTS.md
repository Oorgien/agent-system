...
## Скиллы разработки и поставки

Источники пакета лежат в корне репозитория и устанавливаются CLI в `.agents/`
целевого проекта:

| источник | куда ставится |
|---|---|
| `agents/` — канон ролей explorer, implementer, reviewer | `.agents/agents/` (+ сгенерированные `.claude/agents/`, `.codex/agents/`) |
| `templates/` — шаблоны `task.md`, `entry.md` | `.agents/state/templates/` |
| `skills/` — поставляемые скиллы `checkpoint`, `migrate-memory` | `.agents/skills/` (чужие скиллы проекта сохраняются) |
| `resources/contract.md` — этот контракт | `.agents/agent-system/contract.md` |

`.agents/` в этом клоне — рабочее место агентов, которые работают над самим CLI
(локальные скиллы, память, состояние задач); она целиком gitignored и в пакет не входит,
как и локальная ссылка `.claude/skills`. Описанное ниже версионирование установленных
определений и состояния относится к подключаемым проектам.
...
python3 tools/check_state.py          # проверить состояние задач и привязки (§2)
...
Работа с задачами и привязками — подкоманды CLI:
...
`task status` и `task list` ничего не пишут. `task bind` — единственная команда,
которая создаёт привязку; молча её не создаёт никто.
...
`agents/*.md` и перегенерировать (в подключённом проекте — `.agents/agents/*.md`,
`gen_agents.py --project <root>`).
